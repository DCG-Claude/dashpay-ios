...
public final class DashSDK {
    ...
    public init(configuration: SPVClientConfiguration = .default) throws {
        ...
        print("🔵 Creating StorageManager...")
        do {
            self.storage = try StorageManager()
            print("✅ StorageManager created")
        } catch {
            print("🔴 StorageManager creation failed: \(error)")
            throw DashSDKError.storageError("Failed to create storage manager: \(error.localizedDescription)")
        }
