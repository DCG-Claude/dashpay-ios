...
final class PersistentIdentity {
    ...
    // MARK: - Key Storage
    // Store private keys as Data to avoid CoreData array issues
    var privateKeysData: Data
    
    var privateKeys: [String] {
        get {
            (try? JSONDecoder().decode([Swift.String].self, from: privateKeysData)) ?? []
        }
        set {
            privateKeysData = (try? JSONEncoder().encode(newValue)) ?? Data()
        }
    }
    ...
    init(
        ...
        self.privateKeysData = (try? JSONEncoder().encode(privateKeys)) ?? Data()
