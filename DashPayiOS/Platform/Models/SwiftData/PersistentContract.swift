...
final class PersistentContract {
    ...
    // MARK: - Metadata
    // Store keywords as comma-separated string to avoid CoreData array issues
    var keywordsString: String
    
    var keywords: [String] {
        get {
            keywordsString.isEmpty ? [] : keywordsString.split(separator: ",").map(String.init)
        }
        set {
            keywordsString = newValue.joined(separator: ",")
        }
    }
    ...
    init(
        ...
        self.keywordsString = keywords.joined(separator: ",")
...
extension PersistentContract {
    ...
    static func predicate(keyword: String) -> Predicate<PersistentContract> {
        #Predicate<PersistentContract> { contract in
            contract.keywordsString.contains(keyword)
        }
    }
    ...
    /// Search predicate for contracts by name or keywords
    static func searchPredicate(query: String, network: String) -> Predicate<PersistentContract> {
        #Predicate<PersistentContract> { contract in
            contract.network == network && (
                contract.name.localizedStandardContains(query) ||
                contract.keywordsString.localizedStandardContains(query) ||
                contract.contractDescription?.localizedStandardContains(query) == true
            )
        }
    }
    
    /// Predicate to find contracts by keywords
    static func keywordsPredicate(keywords: [String], network: String) -> Predicate<PersistentContract> {
        #Predicate<PersistentContract> { contract in
            contract.network == network && keywords.allSatisfy { keyword in
                contract.keywordsString.localizedStandardContains(keyword)
            }
        }
    }
    
    /// Predicate to find contracts by owner ID string
    static func ownerPredicate(ownerId: String, network: String) -> Predicate<PersistentContract> {
        #Predicate<PersistentContract> { contract in
            contract.network == network && contract.ownerIdString.contains(ownerId)
        }
    }
}