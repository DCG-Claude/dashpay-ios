...
final class PersistentDocument {
    ...
    init(
        ...
        network: String = PlatformNetwork.testnet.rawValue
    ) {
...
extension PersistentDocument {
    ...
    static func from(_ model: DocumentModel) -> PersistentDocument {
        ...
        if let dppDoc = model.dppDocument {
            ...
            // Note: DPPDocument doesn't have deletedAtBlockHeight
            
            persistent.createdAtCoreBlockHeight = dppDoc.createdAtCoreBlockHeight.map { Int32($0) }
            persistent.updatedAtCoreBlockHeight = dppDoc.updatedAtCoreBlockHeight.map { Int32($0) }
            persistent.transferredAtCoreBlockHeight = dppDoc.transferredAtCoreBlockHeight.map { Int32($0) }
            // Note: DPPDocument doesn't have deletedAtCoreBlockHeight
        }
    ...
    static func from(_ dppDocument: DPPDocument, contractId: String, documentType: String) -> PersistentDocument {
        ...
        let persistent = PersistentDocument(
            ...
            isDeleted: false // DPPDocument doesn't have deletedAt
        )
        
        // Set timestamps
        persistent.transferredAt = dppDocument.transferredDate
        // DPPDocument doesn't have deletedDate
        ...
        // DPPDocument doesn't have deletedAtBlockHeight
        
        persistent.createdAtCoreBlockHeight = dppDocument.createdAtCoreBlockHeight.map { Int32($0) }
        persistent.updatedAtCoreBlockHeight = dppDocument.updatedAtCoreBlockHeight.map { Int32($0) }
        persistent.transferredAtCoreBlockHeight = dppDocument.transferredAtCoreBlockHeight.map { Int32($0) }
        // DPPDocument doesn't have deletedAtCoreBlockHeight
